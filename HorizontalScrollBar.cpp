...
HorizontalScrollBar::HorizontalScrollBar(int x, int y, int width, int height, int minValue, int maxValue, int defaultValue) {
	this->x = x; this->y = y;
	...
	this->tabWidth = this->height-(bevel*2);
}

void HorizontalScrollBar::testEvent(int fingerX, int fingerY) {
	int tabPosX = this->x + calcPosFromValue(this->curValue); int tabPosY = this->y + bevel;
	...
	if (dist < 500 && isLocked) {
		tabColor[0]=tabColor[2]=0; tabColor[1]=1;
		curValue = calcValueFromPos(fingerX-(this->x+bevel));
		//printf("%d\n", curValue);
...
void HorizontalScrollBar::render() {
	glPushMatrix();
//	glLoadIdentity();
	...
	glTranslatef(calcPosFromValue(curValue)-this->tabWidth/2.0, bevel, -0.5);
	//printf("cur:%d - %d\n", curValue, calcPosFromValue(curValue));
	glColor3f(tabColor[0],tabColor[1],tabColor[2]);
	glPushMatrix();
        glBegin(GL_TRIANGLE_STRIP);
            glVertex2f(0.0, 0.0);
            glVertex2f(this->height-(bevel*2), 0.0);
            glVertex2f(0.0, this->height-(bevel*2));
            glVertex2f(this->height-(bevel*2), this->height-(bevel*2));
        glEnd();
	glPopMatrix();
...
double HorizontalScrollBar::getElapsedTime(LARGE_INTEGER start) {
...
int HorizontalScrollBar::calcPosFromValue(int value) {
	float ratio = float(value-this->minValue) / float(this->maxValue-this->minValue);
	
	float posMin = bevel+((float)this->tabWidth/2.0);
	float posMax = this->width - posMin;
	
	return ratio * float(posMax-posMin)+posMin;
}

int HorizontalScrollBar::calcValueFromPos(int pos) {
	float posMin = bevel+((float)this->tabWidth/2.0);
	float posMax = this->width - posMin;
